...
from dataset import RestoreDataset, ValDataset, make_split          # noqa: E402
...
def parse():
    ...
    p.add_argument("--blur-p", type=float, default=None,
                   help="override degradation blur probability (default: degrade.TRAIN value). "
...
def main():
    ...
    train_ids, val_ids = make_split(gt_dir, n_val=a.n_val, seed=a.seed)
    if a.smoke:
    ...
    if a.blur_p is not None:
        deg_cfg["blur_p"] = a.blur_p
        print(f"degradation override: blur_p={a.blur_p}")
    ...
    crit = RestorationLoss(mode=a.loss, w_ssim=a.w_ssim, w_grad=a.w_grad).to(device)
